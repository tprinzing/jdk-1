...
/**
 * A JFR event for datagram receive operations.  This event is mirrored in
 * {@code jdk.jfr.events.DatagramReceiveEvent } where the metadata for the event is
 ...
 */
public class DatagramReceiveEvent extends Event {
    ...
    /**
     * Actually commit an event.  The implementation
     ...
     */
    public static void filteredCommit(long start, long nbytes, SocketAddress remote) {
        long duration = timestamp() - start;
        if (shouldCommit(duration)) {
            nbytes = nbytes < 0 ? 0 : nbytes;
            if (remote instanceof InetSocketAddress isa) {
                commit(start, duration, isa.getHostString(), isa.getAddress().getHostAddress(), isa.getPort(), nbytes);
            }
        }
    }
